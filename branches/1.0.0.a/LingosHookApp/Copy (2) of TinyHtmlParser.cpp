...
{
    os << "\nType = " << type;
    os << "\nTag = " << "[" << start << "," << end << "]" << html.substr(start, end - start + 1);
    if(vstart != 0)
        os << "\nValue = " << "[" << vstart << "," << vend << "]" << html.substr(vstart, vend - vstart + 1);
    os << std::endl;
}
//
...
int CDocumentObject::PushValueData(const TinyHtmlParser::CParserData &data, CDocumentObject::TDataStack &datastack) const
{
    if(datastack.size() == 0)
    {
        THROW_EXCEPTION(EN_DOCUMENT_FORMATERROR, L"Miss '<' before got value - pos : " << data.start);
        return -1;
    }
    ...
    {
        prev.vstart2 = data.start;
        prev.vend2 = data.end;
    }
    else
    {
        THROW_EXCEPTION(EN_DOCUMENT_FORMATERROR, L"Type does match : " << prev.type << " - pos : " << data.start);
        return -1;
    }
...
int CDocumentObject::PushTagData(const std::wstring& html, CParserData& data, CDocumentObject::TDataStack& datastack, CDocumentObject::TNodeQueue& nodeque) const
{
    ...
    else if(data.type == CParserData::DT_END)
    {
        if(datastack.size() == 0)
        {
            THROW_EXCEPTION(EN_DOCUMENT_FORMATERROR, L"Miss start-tag before end-tag - pos : " << data.start);
            return -1;
        }
...
/*
    ...
    if(data.type == CParserData::DT_TAG)
    {
        ...
        //    TNodeData node;
        //    node.tag = data;
        //    node.tag.type = CParserData::DT_DONE;
        ...
        if(datastack.size() > 0 && datastack.top().type == CParserData::DT_VALUE)
        {
            ...
            //if(datastack.top().type == CParserData::DT_VALUE)
            //{
            //    data.end = datastack.top().end;
            //    datastack.pop();
            //}

            while(datastack.size() > 0 && datastack.top().type == CParserData::DT_VALUE)
            {
                data.end = datastack.top().end;
                datastack.pop();
            } 
            
            if(datastack.size() > 0 && datastack.top().type != CParserData::DT_TAG && datastack.top().type != CParserData::DT_TAG_VALUE)//for special condition <t1>v1<t2>v2</t2>v11<t3>v3</t3>v111</t1>
            {
                THROW_EXCEPTION(EN_DOCUMENT_FORMATERROR, L"Wrong tag type : " << datastack.top().type << L" - pos : " << datastack.top().start);
                return -1;
            }
    ...
    else if(data.type == CParserData::DT_END)
    {
        ...
        while(datastack.size() > 0 && datastack.top().type == CParserData::DT_VALUE)
        {
            if(node.value.start == 0)
                node.value = datastack.top();
            else
                node.value.end = datastack.top().end;
            datastack.pop();
        }    
        ...
        //    if(datastack.size() == 0)
        //    {
        //        THROW_EXCEPTION(EN_DOCUMENT_FORMATERROR, L"Miss tag before value.");
        //        return -1;
        //    }
    ...
    else if(data.type == CParserData::DT_DONE)
    {
        if(datastack.size() > 0 && datastack.top().type == CParserData::DT_VALUE)
        {
            ...
            if(datastack.top().type == CParserData::DT_VALUE)
            {
                ...
                {
                    CParserData& da = datastack.top();
                    da.Show(std::wcerr, html);
                    datastack.pop();
                }
                return -1;
            }
...
//        datastack.push(data);
    ...
    {
        THROW_EXCEPTION(EN_DOCUMENT_FORMATERROR, L"Wrong tag type : " << datastack.top().type << L" - pos : " << data.start);
        return -1;
    }
    return 0;
*/
...
int CDocumentObject::PreParserBroken(const std::wstring& html, TDataStack& datastack, TNodeQueue& nodeque) const
{
    while(datastack.size() > 0)
    {
        ...
        else if(data.type == CParserData::DT_VALUE)
        {
            ...
            {
                THROW_EXCEPTION(EN_DOCUMENT_FORMATERROR, L"Wrong tag : " << data.type << L" pos : " << data.start);
                return -1;
            }
            datastack.pop();
...
int CDocumentObject::CheckTag(const std::wstring& html, const CParserData& tag, const CParserData& end) const
{
    ...
    {
        THROW_EXCEPTION(EN_DOCUMENT_FORMATERROR, L"do NOT match tag : " << str << L" and " << html.substr(end.start + 2, end.end - end.start - 2));
        return -1;
    }
   return 0;
}
...
CElementObject* CDocumentObject::MakeElement(const std::wstring& html, const CDocumentObject::TNodeData &node, CElementObject *parent, CElementObject *sibling) const
{
    ...
    {
        THROW_EXCEPTION(EN_DOCUMENT_FORMATERROR, L"Wrong Tag Type : " << node.tag.type);
        return NULL;
    }
    ...
    if(node.value.type == CParserData::DT_VALUE)
    {
        ele->type = ET_ELEMENT;
        if(node.tag.type == CParserData::DT_TAG)
            ele->value = html.substr(node.value.start, node.value.end - node.value.start + 1);
        else
            ele->value = html.substr(node.tag.vstart, node.tag.vend - node.tag.vstart + 1) + L"%" + html.substr(node.value.start, node.value.end - node.value.start + 1);
    }
