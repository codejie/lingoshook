...
DataCleanupDialog::DataCleanupDialog(CDBAccess* dbaccess, CDictObject* dict, wxWindow* parent, int id, const wxString& title, const wxPoint& pos, const wxSize& size, long style):
...
int DataCleanupDialog::RemoveNotMatchData(int& found)
{
	...
	try
	{
		...
		if(total > 0)
		{
			sql = wxT("SELECT WordTable.WordID, SrcDataTable.HTML FROM WordTable, SrcDataTable WHERE (SELECT SrcDataTable.SrcID FROME SrcDataTable WHERE WordTable.SrcID = SrcDataTable.SrcID)");
			res = _objDBAccess->Database().ExecuteQuery(sql);
			if(!res.IsOk())
				return -1;
			found = 0;
